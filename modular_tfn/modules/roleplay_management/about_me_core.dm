...
// -- All persistent data lives in SSRPmanagement (ssrpmanagement.dm)
// ==============================================================================
...
/datum/component/about_me/Destroy()
    if (character_key)
        SSrpmanagement.remove_key_from_all_groups(character_key)
        SSrpmanagement.unregister_aboutme_component(src)
    owner = null
    ..()
...
/datum/component/about_me/proc/get_aboutme_datum(character_key_override = null)
    ...
    return SSrpmanagement.get_aboutme_datum_for_key(key)

/datum/component/about_me/proc/get_record()
	if (!src.character_key) return null
	return SSrpmanagement.get_aboutme_record(src.character_key)
...
/datum/component/about_me/proc/get_full_payload(mob/living/carbon/human/user)
    ...
    if (!(src in GLOB.aboutme_components))
        SSrpmanagement.register_aboutme_component(src)

    var/datum/aboutme_record/R = get_aboutme_datum()
    if (!R)
        R = SSrpmanagement.ensure_aboutme_datum_for_key(character_key, owner)
...
/datum/component/about_me/proc/prompt_vote_on_group(datum/group_vote/V)
    ...
    var/datum/group/G = SSrpmanagement.get_group_by_key(V.group_id)
