...
//Could use this to offload information that is very unlikely to change.
...
/datum/component/about_me/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if ("edit_overview")
			return prompt_edit_overview(user)
		if ("manage_groups")
			return prompt_manage_groups(user)
		if ("change_relationship")
			return prompt_change_relationship(user)
		if ("interact_chronicle")
			return prompt_interact_chronicle(user)
		if ("manage_memories")
			return prompt_manage_memories(user)
