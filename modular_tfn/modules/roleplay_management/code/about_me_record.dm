...
/datum/aboutme_record/proc/get_ui_overview_data(mob/living/carbon/human/owner)
	...
	// Kindred
	if (iskindred(owner))
		// Always prefer mob's .clan var, fallback to species' clan var if for some reason mob.clan is missing
		var/datum/vampire_clan/clan = owner.clan
		species_block["clan"] = clan.name
		// Optionally add more clan info
		if (clan)
