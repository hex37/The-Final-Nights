...
/datum/component/about_me/proc/prompt_edit_overview_status(mob/user)
	...
	if (choice == "Goal")
		var/new_goals = tgui_input_text(user, "Ambition or goal?", "Edit Goals", R.edit_goals)
		if (!isnull(new_goals))
			R.set_goals(html_decode(new_goals))
			to_chat(user, "<span class='notice'>Updated goals.</span>")
		return src.prompt_edit_overview_status(user)
	if (choice == "Quote")
		var/new_quote = tgui_input_text(user, "Your character's personal quote", "Edit Quote", R.edit_personal_quote)
		if (!isnull(new_quote))
			R.set_goals(html_decode(new_quote))
...
/datum/component/about_me/proc/prompt_edit_overview_other(mob/user)
	...
	if (choice == "Gender(Pronouns)")
		var/new_gender = tgui_input_text(user, "Gender/Pronouns", "Edit Gender", R.edit_gender)
		if (!isnull(new_gender))
			R.set_gender(html_decode(new_gender))
			to_chat(user, "<span class='notice'>Updated gender/pronouns.</span>")
		return src.prompt_edit_overview_other(user)
	if (choice == "Physical Description")
		var/new_desc = tgui_input_text(user, "What do you look like?", "Edit Description", R.edit_physical_desc)
		if (!isnull(new_desc))
			R.set_physical_desc(html_decode(new_desc))
