// ================================================================
// RP Management Subsystem - Chronicles (ssrpmanagement_chronicles.dm)
// ================================================================
// Handles:
...
// ================================================================
// ---------------- REGISTRATION ----------------
/datum/controller/subsystem/rpmanagement/proc/register_chronicle(datum/chronicle/C)
	if (C?.id && is_valid_key(C.id))
		GLOB.chronicles[C.id] = C

/datum/controller/subsystem/rpmanagement/proc/unregister_chronicle(datum/chronicle/C)
	if (C?.id)
		GLOB.chronicles -= C.id

/datum/controller/subsystem/rpmanagement/proc/add_chronicle_entry(entry)
	GLOB.chronicles += entry

/datum/controller/subsystem/rpmanagement/proc/get_all_chronicles()
	return GLOB.chronicles

// ---------------- LOOKUP ----------------
/datum/controller/subsystem/rpmanagement/proc/get_chronicle_by_key(key)
	return is_valid_key(key) ? GLOB.chronicles[key] : null

// ---------------- PERSONAL CHRONICLE ----------------
/datum/controller/subsystem/rpmanagement/proc/ensure_personal_chronicle(character_key, mob/living/carbon/human/owner)
